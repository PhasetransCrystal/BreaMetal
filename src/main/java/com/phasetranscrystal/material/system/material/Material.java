...
public class Material {
    ...
    public Material(ResourceLocation materialId, int x16color, IMaterialFeature<?>... fIns) {
        ...
        for (IMaterialFeature<?> feature : fIns) {
            features.put(feature.getType().value(), feature);
        }
        ModBusConsumer.materialModifyCache.featureAttach.get(materialId).forEach(feature -> features.put(feature.getType().value(), feature));
        ...
        Set<ResourceLocation> existingIds = features.keySet().stream().map(MaterialFeatureType::getResourceKey).map(ResourceKey::location).collect(Collectors.toSet());
        ...
        do {
            ...
            if (!toRemove.isEmpty()) {
                ...
                toRemove.stream().map(MaterialFeatureType::getResourceKey).map(ResourceKey::location).forEach(existingIds::remove);
        ...
        if(!typeExceptions.isEmpty()){
            typeExceptions.stream().map(MaterialFeatureType::getResourceKey).map(ResourceKey::location).forEach(typeExceptionRequire::remove);
        ...
        this.toTypes = toFeature.keySet().stream().map(MaterialFeatureType::types).flatMap(HashSet::stream).collect(ImmutableSet.toImmutableSet());
